...
from core.persistence import append_order_recovery_record, locked_json_update, read_json_safe
...
def _positive_float(value, label: str, symbol: str):
    try:
        ...
        logger.error(f"Invalid {label} for {symbol.upper()}: {value!r}. Auto-sell skipped.")
        return None
    if parsed <= 0:
        logger.error(f"Invalid {label} for {symbol.upper()}: {parsed}. Auto-sell skipped.")
        return None
    return parsed
...
def record_order_recovery(
    ...
    try:
        ...
        logger.error(f"CRITICAL: Failed to write order recovery journal for {symbol.upper()}: {e}")
        return False

def save_position(symbol: str, buy_price: float, amount_coin: float, tp1: float, tp2: float, sl: float, trade_type: str = "EARLY", mode: str = "paper") -> bool:
    ...
    try:
        ...
        return False
    logger.info(f"Posisi disimpan: {symbol.upper()} | Mode: {final['mode'].upper()} | Beli: {final['buy_price']:,.0f} | SL: {sl:,.0f} | TP1: {tp1:,.0f}")
    return True
...
def update_position_after_sell(
    ...
    def update(current):
        ...
        current_positions = current.get("positions") if isinstance(current.get("positions"), dict) else current
        if isinstance(current_positions, dict) and symbol in current_positions:
            ...
            target = current
        positions = target.get("positions") if isinstance(target.get("positions"), dict) else target
        ...
        if str(position.get("status", "OPEN")).strip().upper() not in {"OPEN", "SELLING"}:
...
def claim_position_for_sell(
    ...
    def update(portfolio):
        ...
        portfolio = portfolio if isinstance(portfolio, dict) else {}
        positions = portfolio.get("positions") if isinstance(portfolio.get("positions"), dict) else portfolio
        ...
        if status != "OPEN":
            ...
            return portfolio
        held_amount = _positive_float(position.get("amount_coin"), "position amount", clean_symbol)
...
def consume_sell_attempt_for_submit(
    ...
    if expected_amount is not None:
        parsed_amount = _positive_float(expected_amount, "expected sell amount", clean_symbol)
        if parsed_amount is None:
            return {"allowed": False, "reason": "Invalid expected sell amount"}
    parsed_price = None
    if expected_price is not None:
        parsed_price = _positive_float(expected_price, "expected sell price", clean_symbol)
        if parsed_price is None:
            return {"allowed": False, "reason": "Invalid expected sell price"}
    normalized_mode = _normalize_mode(expected_mode) if expected_mode is not None else None
    ...
    def update(portfolio):
        ...
        canonical_mode = _normalize_mode(position.get("selling_mode", position.get("mode", "paper")))
...
def finalize_position_sell(
    ...
    def update(portfolio):
        ...
        owns_selling = status == "SELLING" and position.get("sell_attempt_id") == sell_attempt_id
        owns_submitting = (
        ...
        if not owns_selling and not owns_submitting:
            raise ValueError(f"Sell attempt does not own position {clean_symbol.upper()}")
        finished_at = datetime.now(timezone.utc).isoformat()
        if not success:
            position.update({
                "status": "FAILED_UNKNOWN" if manual_reconciliation_required else "FAILED",
                "sell_error": error or "Sell execution failed",
                "selling_finished_at": finished_at,
                "manual_reconciliation_required": bool(manual_reconciliation_required),
                "submission_status": submission_status or ("UNKNOWN" if manual_reconciliation_required else "FAILED"),
            })
            finalized = True
            return portfolio
...
def execute_position_sell(
    ...
    if not clean_symbol or parsed_price is None:
        ...
        return {"success": False, "mode": "blocked", "action": "sell", "symbol": clean_symbol, "error": error, "reason": error}

    claim = claim_position_for_sell(clean_symbol, reason, parsed_price, actor=actor, amount=sold_amount)
    ...
    held_amount = _positive_float(claimed_position.get("amount_coin"), "claimed amount", clean_symbol)
    requested_amount = _positive_float(claimed_position.get("selling_amount"), "claimed sell amount", clean_symbol)
    if held_amount is None or requested_amount is None or requested_amount > held_amount:
    ...
    try:
        ...
        return {"success": False, "mode": "blocked", "action": "sell", "symbol": clean_symbol, "error": str(e), "reason": str(e)}
    ...
    unknown = bool(result.get("submission_status") == "UNKNOWN" or result.get("manual_reconciliation_required"))
